...
import me.alikuxac.vortexia.core.core.storage.NamespacedStorageManager;
...
public class CoreVortexiaAPI implements VortexiaAPI {
    ...
    @Override
    public StorageManager getStorageManager(Plugin plugin) {
        if (plugin == null) {
            return getStorageManager();
        }
        String namespace = plugin.getName().toLowerCase().replace("-", "_");
        return new NamespacedStorageManager(this.plugin.getStorageManager(), namespace);
    }
    ...
    @Override
    public WirelessNetworkRegistry getWirelessRegistry() {
        return plugin.getWirelessRegistry();
    }

    @Override
    public WailaManager getWailaManager() {
        return plugin.getWailaManager();
    }

    @Override
    public GridManager getGridManager() {
        return plugin.getGridManager();
    }

    @Override
    public CustomRecipeManager getCustomRecipeManager() {
        return plugin.getCustomRecipeManager();
    }
    ...
    @Override
    public CompletableFuture<Optional<String>> getBlockMetadata(Location loc, String key) {
        String locKey = loc.getWorld().getName() + "_" + loc.getBlockX() + "_" + loc.getBlockY() + "_" + loc.getBlockZ();
        return plugin.getStorageManager().getMetadata(UUID.nameUUIDFromBytes(locKey.getBytes()), key);
    }

    @Override
    public CompletableFuture<Void> setBlockMetadata(Location loc, String key, String value) {
        String locKey = loc.getWorld().getName() + "_" + loc.getBlockX() + "_" + loc.getBlockY() + "_" + loc.getBlockZ();
        return plugin.getStorageManager().setMetadata(UUID.nameUUIDFromBytes(locKey.getBytes()), key, value);
    }

    @Override
    public CompletableFuture<Void> removeBlockMetadata(Location loc, String key) {
        String locKey = loc.getWorld().getName() + "_" + loc.getBlockX() + "_" + loc.getBlockY() + "_" + loc.getBlockZ();
        return plugin.getStorageManager().removeMetadata(UUID.nameUUIDFromBytes(locKey.getBytes()), key);
    }

    @Override
    public void openGuide(Player player, String category) {
        if (category == null || category.isEmpty()) {
            plugin.getGuideGUI().openMain(player);
        } else {
            plugin.getGuideGUI().openCategory(player, category);
        }
    }
}
