...
package me.alikuxac.vortexia.core.core.storage.impl;
...
import me.alikuxac.vortexia.core.core.storage.IStorage;
import me.alikuxac.vortexia.core.core.storage.StorageConfig;
...
public class SQLiteStorage implements IStorage {
  ...
  private static final String CREATE_INVENTORY_TABLE = """
      CREATE TABLE IF NOT EXISTS vortexia_inventory_sync (
  ...
  private static final String CREATE_INDEX_INVENTORY_TIMESTAMP = "CREATE INDEX IF NOT EXISTS idx_inv_timestamp ON vortexia_inventory_sync(citizen_id, timestamp)";
  ...
  private static final String INSERT_INVENTORY_SNAPSHOT = """
      INSERT INTO vortexia_inventory_sync (citizen_id, inventory_data, ender_chest_data, timestamp)
      VALUES (?, ?, ?, ?)
      """;

  private static final String SELECT_LATEST_INVENTORY = """
      SELECT citizen_id, inventory_data, ender_chest_data, timestamp
      FROM vortexia_inventory_sync
      WHERE citizen_id = ?
  ...
  private static final String SELECT_INVENTORY_BY_TIMESTAMP = """
      SELECT citizen_id, inventory_data, ender_chest_data, timestamp
      FROM vortexia_inventory_sync
      WHERE citizen_id = ? AND timestamp = ?
      """;

  private static final String DELETE_OLD_INVENTORIES = """
      DELETE FROM vortexia_inventory_sync
      WHERE citizen_id = ? AND timestamp NOT IN (
          SELECT timestamp FROM vortexia_inventory_sync
          WHERE citizen_id = ?
          ORDER BY timestamp DESC
          LIMIT ?
      )
  ...
  private void createTables() throws SQLException {
    try (Connection conn = dataSource.getConnection();
      ...
      // 4. Create inventory table and index
      stmt.execute(CREATE_INVENTORY_TABLE);
      stmt.execute(CREATE_INDEX_INVENTORY_TIMESTAMP);
    }
  }
  ...
  @Override
  public CompletableFuture<Void> saveInventorySnapshot(String citizenId, String inventoryData, String enderChestData) {
    return CompletableFuture.runAsync(() -> {
      try (Connection conn = dataSource.getConnection();
          PreparedStatement stmt = conn.prepareStatement(INSERT_INVENTORY_SNAPSHOT)) {
        stmt.setString(1, citizenId);
        stmt.setString(2, inventoryData);
        stmt.setString(3, enderChestData);
        stmt.setLong(4, System.currentTimeMillis());
        stmt.executeUpdate();
      } catch (SQLException e) {
        throw new RuntimeException("Failed to save inventory snapshot", e);
      }
    });
  }

  @Override
  public CompletableFuture<Optional<InventorySnapshot>> getLatestInventorySnapshot(String citizenId) {
    return CompletableFuture.supplyAsync(() -> {
      try (Connection conn = dataSource.getConnection();
          PreparedStatement stmt = conn.prepareStatement(SELECT_LATEST_INVENTORY)) {
        stmt.setString(1, citizenId);
        try (ResultSet rs = stmt.executeQuery()) {
          if (rs.next()) {
        ...
        throw new RuntimeException("Failed to get latest inventory snapshot", e);
      }
      return Optional.empty();
    });
  }
  ...
  @Override
  public CompletableFuture<List<InventorySnapshot>> getInventorySnapshots(String citizenId, int limit) {
    return CompletableFuture.supplyAsync(() -> {
      List<InventorySnapshot> snapshots = new ArrayList<>();
      try (Connection conn = dataSource.getConnection();
          PreparedStatement stmt = conn.prepareStatement(SELECT_ALL_INVENTORY)) {
        stmt.setString(1, citizenId);
        stmt.setInt(2, limit);
        try (ResultSet rs = stmt.executeQuery()) {
          while (rs.next()) {
        ...
        throw new RuntimeException("Failed to get inventory snapshots", e);
      }
      return snapshots;
    });
  }
  ...
  @Override
  public CompletableFuture<Optional<InventorySnapshot>> getInventorySnapshotByTimestamp(String citizenId, long timestamp) {
    return CompletableFuture.supplyAsync(() -> {
      try (Connection conn = dataSource.getConnection();
          PreparedStatement stmt = conn.prepareStatement(SELECT_INVENTORY_BY_TIMESTAMP)) {
        stmt.setString(1, citizenId);
        ...
        throw new RuntimeException("Failed to get inventory snapshot by timestamp", e);
      }
      return Optional.empty();
    });
  }
  ...
  @Override
  public CompletableFuture<Void> cleanOldInventorySnapshots(String citizenId, int keepLimit) {
    return CompletableFuture.runAsync(() -> {
      try (Connection conn = dataSource.getConnection();
          PreparedStatement stmt = conn.prepareStatement(DELETE_OLD_INVENTORIES)) {
        stmt.setString(1, citizenId);
        stmt.setString(2, citizenId);
        stmt.setInt(3, keepLimit);
        stmt.executeUpdate();
      } catch (SQLException e) {
        throw new RuntimeException("Failed to clean old inventory snapshots", e);
      }
    });
  }
